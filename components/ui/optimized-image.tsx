...
export function OptimizedImage({
  ...
  const blurDataURL = "data:image/jpeg;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==";
  ...
  useEffect(() => {
    ...
    async function fetchMetadata() {
      try {
        ...
        if (data) {
          setMetadata(data);
          const thumbnail = data.thumbnails?.find((t: ImageThumbnail) => t.size === size);
          if (thumbnail) {
            setFallbackSrc(thumbnail.path);
          }
        }
      } catch (error) {
        console.warn(`Impossible de charger les métadonnées pour l'image: ${src}`, error);
      }
    }
  ...
  const handleError = () => {
    ...
    if (fallbackSrc && fallbackSrc !== src) {
      console.warn(`Impossible de charger l'image optimisée, utilisation de la source originale: ${src}`);
      setFallbackSrc(src);
    } else {
